 ...
 * through a tool that was never meant to reach one.
 */
 ...
 *
 * @remarks
 * Generous enough for a full audit or a long research memo, bounded so one call can't push an
...
/**
 * The kinds of artifact an agent can save.
 *
 * @remarks
 * A closed set rather than free text: the kind travels with the id into another agent's brief, and a
 * reader that knows it is holding an `seo-audit` treats it differently from a `research-notes`. Add
 * a kind here when a new handoff shape appears rather than letting callers invent one.
 *
 * `deliverability-audit` is the email agent's parallel to `seo-audit`: a list and sending-domain
 * review whose findings another agent acts on. A campaign itself is not a kind, because it lives in
 * Resend and travels as a link rather than as a document.
 */
 ...
 *
 * @remarks
 * Readable rather than opaque, because these ids show up in briefs and in logs, and
 * `seo-audit-pricing-page-k3f9qz` is far easier to reason about than a bare UUID. The random suffix
 * keeps two audits of the same page from colliding.
 *
 ...
 * a model-supplied id inside the reserved namespace. Callers treat `null` as "not found" rather than
