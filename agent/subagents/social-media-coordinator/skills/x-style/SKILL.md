...
- `references/banned-words.json`: the words to avoid. The `lint_against_style` tool checks a draft against this list (surface `x`), so run the tool rather than reading the file.
